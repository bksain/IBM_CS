...
#from CS_Net_plus_IFBN_readout_nonlinear import CS_reconstruction, Discriminator, Encoder, ZBottleneck
...
weight_save_path = './models/CSNet+_0.1_matrix_IFBN2_n3'
cs_ratio = 0.1
max_CS_ratio = 0.3
LR_size = 32
...
def patial_cubic(image):
    ...
    fill = cv2.resize(temp, (len(temp[0]) // reduction_ratio, len(temp) // reduction_ratio),
...
def test(model, mode):
    ...
    for i, CS in enumerate(CS_ratio):
        ...
        for i, name in enumerate(val_list):
            ...
            recon, _, _ = model(input_val, CS, mode)
...
init_weights(discriminator, 'orthogonal')

zBottleneck = ZBottleneck(LR_size, max_CS_ratio)
zBottleneck = nn.DataParallel(zBottleneck).cuda()
init_weights(zBottleneck, 'orthogonal')

generator = Encoder(LR_size, max_CS_ratio)
generator = nn.DataParallel(generator).cuda()
init_weights(generator, 'orthogonal')

model = CS_reconstruction(F, LR_size, phase=phase, G=generator, Z=zBottleneck, max_CS_ratio=max_CS_ratio)
...
for epoch in range(start_point, iter):
    ...
    print('mean : %4f, max : %4f, min : %4f'%(zBottleneck.module.lamb.mean().item(),zBottleneck.module.lamb.max().item(),zBottleneck.module.lamb.min().item()))
    ...
    with torch.no_grad():
        psnr, ssim = test(model, 0)
        #psnr1, ssim1 = test(model, 1)
        psnr1, ssim1 = [[0],[0]]

        print(' 0.1 0 : %.2f, 0.1 1 : %.2f'%(psnr[0],psnr1[0]))
    output_data = '[epoch : %d] loss : %f, main_loss : %f, rip_H_loss : %f, G_loss : %f, Encoder_train : %f, PSNR : %.2f, SSIM : %.4f\n' % (
        epoch, running_loss, running_main, running_rip_H, running_y, running_E, psnr[0], ssim[0])
